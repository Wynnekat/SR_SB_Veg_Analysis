...
bray_curtis_cover_rain.plot <- ggplot()+
  geom_jitter(data=Cover_rain_dis_results, aes(x = Site, y = dissimilarity_bray, color = Site), show.legend = FALSE, width = .2, size =2) +
  geom_point(data = Cover_rain_dissimilarity, aes(x = site, y = response), show.legend = FALSE, size = 3.5)+
  geom_errorbar(data = Cover_rain_dissimilarity, aes(x= site, y = response, ymin = lower, ymax = upper), width = 0.15, cex = .75)+
  theme_classic()+
...
bray_curtis_cover_rain.plot <- bray_curtis_cover_rain.plot + draw_image(
  "~/SR_SB_Veg_Analysis/Chapter_2.Seed_Bank_Seed Rain_and_Aboveground_Vegetation/Images/AMBART_seed.png",
  x = .6, y = .5, width = .25
)

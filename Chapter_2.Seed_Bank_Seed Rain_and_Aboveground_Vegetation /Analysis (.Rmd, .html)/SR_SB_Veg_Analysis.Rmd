...
response <- c(0.501, 0.668, 0.682, 0.614)
lower <- c(0.452, 0.619,  0.632, 0.565) 
upper <- c(0.551, 0.718, 0.731,  0.664)
site <- c("PFCA 1", "PFCA 2", "PFCA 3", "TP")
...
response <- c(0.217, 0.224, 0.520, 0.289)
lower <- c(0.137, 0.145,  0.440, 0.209) 
upper <- c(0.297, 0.304, 0.600,  0.369)
site <- c("PFCA 1", "PFCA 2", "PFCA 3", "TP")
...
response <- c(0.0333,  0.0992, 0.4656, 0.2853)
lower <- c(-0.0844, -0.0185,  0.3479, 0.1676) 
upper <- c(0.151, 0.217, 0.583,  0.403)
site <- c("PFCA 1", "PFCA 2", "PFCA 3", "TP")
...
### Include certain trait information (annual vs. perennial, provenance)

tern.plot.data.traits <- left_join(tern.plot.data, traits)
tern.plot.data.traits <- tern.plot.data.traits [, c("Site", "SPP6", "Aboveground", "Seed_Bank", "Seed_Rain", "Provenance", "Life_History_Lumped")]


# Removed 5 species at the genus level that could have either traits
tern.plot.data.traits <- na.omit(tern.plot.data.traits)

### Tucker Prairie


tern.plot.data_TP <- tern.plot.data.traits %>% 
  filter(Site == "TP")
...
TP.tern.plot <- ggtern(data=tern.plot.data_TP,aes(x=Aboveground,y=Seed_Bank, z=Seed_Rain))+
  geom_mask() +
...
#### Prairie Fork 1
tern.plot.data_PFCA_1 <- tern.plot.data.traits %>% 
  filter(Site == "PFCA 1")
...
#### Prairie Fork 2
tern.plot.data_PFCA_2 <- tern.plot.data.traits %>% 
  filter(Site == "PFCA 2")
...
#### Prairie Fork 3
tern.plot.data_PFCA_3 <- tern.plot.data.traits %>% 
  filter(Site == "PFCA 3")
...
# Ok I think I'm going to color the points based on native vs. introduced and change the shape based on annual vs. perennial
```
...
### Number of seedlings at each transect
...
#### Total Species Richness - Aboveground
...
#### Total Species Richness - soil seed banks
...
#### Native Species Richness - soil seed banks
